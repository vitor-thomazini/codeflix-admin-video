...
import vitor.thomazini.codeflixadminvideo.application.genre.create.CreateGenreOutput;
...
import vitor.thomazini.codeflixadminvideo.application.genre.update.UpdateGenreOutput;
...
public class GenreAPITest {
    ...
    public void givenAValidId_whenCallsGetGenreById_shouldReturnGenre() throws Exception {
        ...
        final var expectedId = aGenre.id().value();
        ...
        response.andExpect(status().isOk())
                ...
                .andExpect(jsonPath("$.created_at", equalTo(aGenre.createdAt().toString())))
                .andExpect(jsonPath("$.updated_at", equalTo(aGenre.updatedAt().toString())))
                .andExpect(jsonPath("$.deleted_at", equalTo(aGenre.deletedAt().toString())));
    ...
    public void givenAnInvalidId_whenCallsGetGenreById_shouldReturnNotFound() throws Exception {
        ...
        // Act
        final var aRequest = get("/genres/{id}", expectedId.value())
                .accept(MediaType.APPLICATION_JSON)
                .contentType(MediaType.APPLICATION_JSON);
        ...
        verify(getGenreByIdUseCase).execute(eq(expectedId.value()));
    }
    ...
    public void givenValidParams_whenCallsListGenres_shouldReturnGenres() throws Exception {
        ...
        response.andExpect(status().isOk())
                ...
                .andExpect(jsonPath("$.items[0].id", equalTo(aGenre.id().value())))
                .andExpect(jsonPath("$.items[0].name", equalTo(aGenre.name())))
                .andExpect(jsonPath("$.items[0].is_active", equalTo(aGenre.isActive())))
                .andExpect(jsonPath("$.items[0].created_at", equalTo(aGenre.createdAt().toString())))
                .andExpect(jsonPath("$.items[0].deleted_at", equalTo(aGenre.deletedAt().toString())));
